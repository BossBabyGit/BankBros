...
function extractUsername(entry, fallback = "Player") {
  return entry?.username ?? entry?.user ?? entry?.name ?? entry?.player ?? entry?.displayName ?? fallback;
}
function extractWagered(entry) {
  const src = entry?.wagered ?? entry?.totalAmount ?? entry?.amount ?? entry?.value ?? entry?.total ?? 0;
...
async function fetchDejenLeaderboard(raceId) {
  ...
  return { schemaVersion: SCHEMA_VERSION, rows, prizes, metadata: { source: "dejen", raceId, fetchedAt: new Date().toISOString(), url } };
}
...
async function fetchCsGoldLeaderboard() {
  ...
  return { schemaVersion: SCHEMA_VERSION, rows, prizes, metadata: { source: "csgold", fetchedAt: new Date().toISOString(), url: endpoint.toString() } };
}
